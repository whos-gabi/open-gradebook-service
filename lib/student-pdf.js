...
const buildAbsenceEntries = (absences) => {
  const absenceMap = absences.reduce((acc, absence) => {
    // In the test SQLite schema, Absence links to Timetable (which links to Subject).
    // In the production schema, Absence also has subjectId/subject, but Timetable->Subject exists too.
    const subject =
      absence.subject ||
      absence.timetable?.subject ||
      absence.timetable?.classCourse?.subject ||
      absence.timetable?.Subject;
    ...
    const subjectKey = subject.id || subject.subjectId || subject.name;
    if (!acc[subjectKey]) {
      acc[subjectKey] = {
        subjectName: subject.name || 'Materie necunoscută',
        dates: [],
      };
    }

    const absenceDateValue = absence.absenceDate || absence.date;
    if (absenceDateValue) {
      acc[subjectKey].dates.push(new Date(absenceDateValue));
    }
    return acc;
  }, {});
...
const exportStudentReport = async (req, res) => {
  try {
    ...
    // Prisma schema differs between prod (Postgres) and tests (SQLite schema.test.prisma).
    // - Prod: Absence -> Timetable -> ClassCourse -> Subject
    // - Test: Absence -> Timetable -> Subject
